...
2. Write to Aiven Postgres via Npgsql raw `INSERT` statements with an explicit
   `Id` column, **preserving primary-key IDs**, in FK-dependency order:
   **Users → Decks → Cards → FavoriteDecks**. No `OVERRIDING SYSTEM VALUE`
   clause is needed because the identity columns are `GENERATED BY DEFAULT AS
   IDENTITY` (that clause only applies to `GENERATED ALWAYS` columns).
